...
This diagram was a diagnosis, unmeasured, for over a year of this package's life. `TRIPLE_TAX_CALCULUS.md` measures it: real token counts on 5 shipped decision points put the compiled slice at a 4668.8x reduction against the raw spec's 2,247,567 tokens. One part of the diagram above genuinely wasn't confirmed, and one part is unconfirmable with a live model at all — both reported plainly rather than left implicit, same reason `SOURCES.md` says what it invented instead of staying silent. The 3-pass raw-FPF path was **never live-tested** — the raw spec is 2,247,567 tokens, past any practical context window, so only its token count was measured, not its actual reasoning behavior; the "3 passes" framing above is untested, not falsified. The live probe that *did* run tested the compiled side only, and found the model self-reports 0 distinct passes on the compiled slice — consistent with the "1 pass" framing, not evidence against the raw side. Separately, cost growth across a multi-step traversal came back `mixed` (+20.5% then -28.4%) rather than the monotonic compounding `WHY_THIS_EXISTS.md` describes — but that reading rests on 2 deltas from a 3-step traversal that terminates early by design, not enough steps to make a strong claim about compounding shape either way. The token-count advantage is real, large, and directly measured. The pass-by-pass mechanism on the raw side remains an untested hypothesis, and the compounding shape is thin evidence, not a settled reversal.
